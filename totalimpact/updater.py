...
def update_active_publisher_items(number_to_update, single_publisher, myredis, mydao):
    all_tiids = []
    all_docs = []
    if single_publisher:
        publishers_to_consider = [single_publisher]
    else:    
        publishers_to_consider = active_publishers.keys()
    for publisher in publishers_to_consider:
...
def main(action_type, number_to_update=35, publisher=None):
    ...
    try:
        if action_type == "active_publishers":
            ...
            tiids = update_active_publisher_items(number_to_update, publisher, myredis, mydao)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--publisher', default=None, type=str, help="Publisher to update.")
    args = vars(parser.parse_args())
    print args
    print "updater.py starting."
    main(args["action_type"], args["number_to_update"], args["publisher"])

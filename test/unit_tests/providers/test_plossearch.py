...
class TestPlossearch(ProviderTestCase):
    ...
    def test_provenance_url(self):
        ...
        expected = 'http://www.plosone.org/search/advanced?queryTerm=&unformattedQuery=everything:"http%3A//hdl.handle.net/10255/dryad.235"'
        assert_equals(provenance_url, expected)

    @http
    def test_metrics(self):
        metrics_dict = self.provider.metrics([self.testitem_metrics])
        expected = {'plossearch:mentions': (2, 'http://www.plosone.org/search/advanced?queryTerm=&unformattedQuery=everything:"http%3A//hdl.handle.net/10255/dryad.235"')}
        print metrics_dict
        for key in expected:
